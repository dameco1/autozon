# Corporate Structure

## Overview

Autozon will operate as an **Austrian GmbH** (Gesellschaft mit beschränkter Haftung) incorporated in Vienna. This document details the planned corporate structure, incorporation process, governance framework, and operational setup for investor due diligence.

---

## Legal Entity

| Field | Detail |
|---|---|
| **Company name** | autozon FlexCo |
| **Legal form** | Flexible Kapitalgesellschaft (FlexKapG / FlexCo) |
| **Governing law** | Flexible Company Act (FlexKapGG), effective 1 January 2024 |
| **Jurisdiction** | Republic of Austria |
| **Registered office** | Vienna, Austria |
| **Commercial register** | Firmenbuch (Handelsgericht Wien) |
| **Minimum share capital** | €10,000 (€5,000 paid-in at incorporation) |
| **Minimum corporate tax** | €125/quarter (€500/year) |
| **Fiscal year** | Calendar year (1 Jan – 31 Dec) |
| **VAT registration** | Austrian UID number (Umsatzsteuer-Identifikationsnummer) |
| **Trade license** | Gewerbeberechtigung for online marketplace / IT services |

### Why FlexCo instead of traditional GmbH?

Since January 2024, Austria offers the **FlexCo (FlexKapG)** as a modern alternative to the GmbH, specifically designed for startups and growth companies:

| Feature | Traditional GmbH | FlexCo (FlexKapG) |
|---|---|---|
| Minimum share capital | €35,000 (€10,000 privileged) | €10,000 |
| Minimum paid-in at start | €17,500 (€5,000 privileged) | €5,000 |
| Privilege time limit | 10 years (then convert) | No limit — permanent |
| Value participation shares | Not available | Up to 24.99% of capital |
| Employee equity (ESOP) | Complex, tax-inefficient | Built-in, tax-advantaged |
| Notarial deed for share transfers | Required | Not required for value shares |
| Conversion | — | Seamless to/from GmbH, tax-neutral |
| Minimum corporate tax | €500/quarter | €125/quarter |

**Recommendation:** Incorporate as FlexCo to benefit from lower capital requirements, built-in employee equity instruments, and startup-friendly governance. Convert to GmbH or AG only if required at later stages.

---

## Entity Status

> ⚠️ **autozon FlexCo is not yet incorporated.** The company is currently in the pre-registration planning phase. All references to "autozon FlexCo" in this document describe the **planned** legal entity. Incorporation will be initiated upon securing seed funding.

### Pre-Launch Phase (Current)

The platform is currently operated as a **pre-incorporation project** by the founding team. Key considerations:

| Aspect | Current Status | Post-Incorporation |
|---|---|---|
| Legal entity | Not yet registered | autozon FlexCo (Austrian FlexKapG) |
| IP ownership | Held by founders personally | Transferred to autozon FlexCo |
| Contracts | Personal liability of founders | Corporate liability |
| Revenue collection | Not active | Via autozon FlexCo |
| Tax obligations | Individual founders | Corporate (KöSt, USt) |

### Incorporation Costs (FlexCo / FlexKapG)

| Item | Cost Range | Notes |
|---|---|---|
| Share capital (paid-in) | €5,000 | Minimum paid-in at start (total capital: €10,000) |
| Notary fees (Notariatsakt) | €500–€900 | Required for FlexCo formation |
| Court registration (Firmenbuch) | ~€150 | Commercial register filing |
| Lawyer (optional) | €0–€500 | Articles of association review |
| Tax advisor setup | €300–€800 | Tax & VAT registration |
| Chamber of Commerce fee | ~€150 | Annual fee (Wirtschaftskammer) |
| **Total initial cost** | **€1,000–€2,000 + €5,000 capital** | One-time formation costs |

### Ongoing Annual Costs

| Item | Cost Range | Notes |
|---|---|---|
| Annual accounting (Steuerberater) | €1,200–€2,500 | Bookkeeping, Jahresabschluss, tax returns |
| Chamber of Commerce fee | ~€150/year | Wirtschaftskammer membership |
| **Total yearly cost** | **€1,500–€2,700** | Ongoing corporate maintenance |

### Estimated Timeline to Operational FlexCo

| Phase | Duration | Key Milestones |
|---|---|---|
| Preparation | Weeks 1–2 | Legal counsel engaged, articles drafted, IP audit |
| Formation | Weeks 2–4 | Notarization, bank account, capital deposit, Firmenbuch filing |
| Registration | Weeks 4–6 | Tax/VAT registration, trade license, WiEReG, SVS |
| Operational | Weeks 6–8 | All registrations complete, FlexCo fully operational |

**Target:** FlexCo fully operational within **60–90 days** of initiating incorporation (contingent on seed funding close).

---

## Ownership Structure

### At Incorporation

| Shareholder | Ownership | Shares | Role |
|---|---|---|---|
| Damir Buljubasic | 60% | 60,000 | Co-Founder & CIO |
| Emina Mukic-Buljubasic | 10% | 10,000 | Co-Founder & CEO |
| Nenad Brankovic | 10% | 10,000 | CFO |
| First Investor (reserved) | 20% | 20,000 | Seed Investor (€200K) |

### Post-ESOP Allocation

| Shareholder | Ownership | Notes |
|---|---|---|
| Damir Buljubasic (Co-Founder & CIO) | 57.1% | |
| Emina Mukic-Buljubasic (Co-Founder & CEO) | 9.5% | |
| Nenad Brankovic (CFO) | 9.5% | |
| First Investor | 19.0% | €200K at €1M valuation |
| ESOP Pool (reserved) | 4.8% | **Value participation shares** (FlexCo Wertbeteiligungsanteile) — no notarial deed required, tax-advantaged for employees |

*See [Cap Table](cap-table.md) for detailed equity scenarios and dilution modeling.*

---

## Governance Framework

### Management Board (Geschäftsführung)

| Role | Name | Responsibility |
|---|---|---|
| **Geschäftsführerin (Managing Director)** | Emina Mukic-Buljubasic | Corporate strategy, operations, legal authority |
| **Prokurist (Authorized Officer)** | Damir Buljubasic | Technology, innovation, business development |

Under Austrian FlexKapGG, the Geschäftsführer has sole authority unless restricted by the articles of association (Gesellschaftsvertrag) or shareholder resolutions. Governance rules are substantively the same as GmbH.

### Shareholder Meeting (Generalversammlung)

| Matter | Approval Threshold |
|---|---|
| Ordinary resolutions | Simple majority (> 50%) |
| Amendment of articles | 75% supermajority |
| Capital increase | 75% supermajority |
| Dissolution | 75% supermajority |
| Appointment / removal of Geschäftsführer | Simple majority |

### Advisory Board (Beirat) — Planned

| Seat | Focus Area | Status |
|---|---|---|
| Automotive industry expert | Market strategy, dealer relationships | 🔍 Recruiting |
| Fintech / marketplace advisor | Platform scaling, unit economics | 🔍 Recruiting |
| Legal / regulatory advisor | Austrian corporate law, GDPR | 🔍 Recruiting |

The Beirat is advisory (not a Aufsichtsrat) and has no statutory authority. Formal appointment planned post-seed.

---

## Incorporation Process

### Step-by-Step

| # | Step | Responsible | Timeline | Status |
|---|---|---|---|---|
| 1 | Draft articles of association (Gesellschaftsvertrag) | Corporate lawyer | Week 1–2 | 📋 Planned |
| 2 | Notarization (Notariatsakt) | Notar (Vienna) | Week 2 | 📋 Planned |
| 3 | Open bank account & deposit share capital | Founders | Week 2–3 | 📋 Planned |
| 4 | Commercial register filing (Firmenbuch) | Corporate lawyer | Week 3 | 📋 Planned |
| 5 | Tax registration (Finanzamt) | Tax advisor | Week 3–4 | 📋 Planned |
| 6 | VAT registration (UID) | Tax advisor | Week 4 | 📋 Planned |
| 7 | Trade license application (Gewerbebehörde) | CEO | Week 4–5 | 📋 Planned |
| 8 | SVS registration (social insurance) | CEO | Week 4 | 📋 Planned |
| 9 | Data protection registration (DSB) | Legal counsel | Week 5 | 📋 Planned |
| 10 | Domain portfolio registration | CEO | Week 5–8 | 📋 Planned |

**Estimated total incorporation cost:** €1,000–€2,000 + €5,000 share capital (paid-in). Ongoing: €1,500–€2,700/year.

### Required Documents

| Document | Purpose |
|---|---|
| Gesellschaftsvertrag (Articles of Association) | Core corporate document defining rights, shares, governance |
| Notariatsakt (Notarial deed) | Legally required for FlexCo formation |
| Musterzeichnung (Specimen signature) | Geschäftsführer signature registration |
| Bank confirmation (Bankbestätigung) | Proof of share capital deposit |
| Firmenbuch application | Commercial register entry |
| Declaration of beneficial owners (WiEReG) | Beneficial ownership register filing |

---

## Regulatory & Compliance

### Required Registrations

| Registration | Authority | Status |
|---|---|---|
| Firmenbuch (Commercial Register) | Handelsgericht Wien | 📋 At incorporation |
| Gewerbeberechtigung (Trade License) | Magistrat der Stadt Wien | 📋 At incorporation |
| UID (VAT Number) | Finanzamt | 📋 At incorporation |
| WiEReG (Beneficial Ownership Register) | BMF | 📋 At incorporation |
| DSB (Data Protection Authority) | Datenschutzbehörde | 📋 At incorporation |
| SVS (Social Insurance) | SVS | 📋 At incorporation |

### Ongoing Compliance Obligations

| Obligation | Frequency | Responsible |
|---|---|---|
| Annual financial statements (Jahresabschluss) | Annual | Tax advisor / auditor |
| Corporate tax return (KöSt) | Annual | Tax advisor |
| VAT returns (UVA) | Monthly / quarterly | Tax advisor |
| Firmenbuch updates | As needed | Corporate lawyer |
| WiEReG updates | As needed | Corporate lawyer |
| GDPR compliance review | Quarterly | DPO / legal counsel |
| Annual trade license renewal | Annual | CEO |

### Tax Overview

| Tax | Rate | Notes |
|---|---|---|
| **Corporate tax (KöSt)** | 23% (from 2024) | On taxable profit |
| **VAT (USt)** | 20% standard | On services; input VAT deductible |
| **Minimum corporate tax** | €125/quarter (€500/year) | FlexCo rate; payable regardless of profit |
| **Capital gains (KESt)** | 27.5% | On distributions to shareholders |
| **Employer contributions** | ~30% of gross salary | Social insurance, payroll tax, etc. |

---

## Intellectual Property

### IP Assets

| Asset | Current Owner | Post-Incorporation Owner | Status |
|---|---|---|---|
| autozon platform (source code) | Founding team | autozon FlexCo | 📋 Transfer at incorporation |
| autozon brand & trademarks | Founding team | autozon FlexCo | 📋 Transfer at incorporation |
| Domain: autozon.at | Founding team | autozon FlexCo | 📋 Transfer at incorporation |
| AI models & training data | Founding team | autozon FlexCo | 📋 Transfer at incorporation |
| Business processes & documentation | Founding team | autozon FlexCo | 📋 Transfer at incorporation |

> **IP Assignment:** All founders will execute IP assignment agreements transferring all project-related intellectual property to autozon FlexCo at incorporation. This is a standard requirement and will be included in the Gesellschaftsvertrag.

### IP Protection Strategy

| Action | Timeline | Status |
|---|---|---|
| Austrian trademark registration (autozon) | At incorporation | 📋 Planned |
| EU trademark registration (EUIPO) | Post-seed | 📋 Planned |
| Domain portfolio (autozon.de, autozon.ch) | Pre-incorporation | 📋 Planned |
| Software copyright documentation | At incorporation | 📋 Planned |
| Trade secret policy | At incorporation | 📋 Planned |

---

## Infrastructure & Operations

### Current (MVP Phase)

| Component | Provider | Cost |
|---|---|---|
| Application hosting | Lovable Cloud | Included in platform |
| Database | Lovable Cloud | Included in platform |
| Authentication | Lovable Cloud | Included in platform |
| Payments | Stripe | Transaction fees only |
| AI inference | OpenAI / Google | Pay-per-use |
| Domain & DNS | Registrar | ~€15/year |

### Planned (Post-Migration to AWS)

| Component | AWS Service | Est. Monthly Cost |
|---|---|---|
| Application hosting | ECS Fargate / Lambda | €80–€200 |
| Database | RDS PostgreSQL | €150–€400 |
| Object storage | S3 | €20–€100 |
| CDN | CloudFront | €30–€100 |
| AI inference | SageMaker / Bedrock | €200–€500 |
| Monitoring | CloudWatch | €30–€50 |
| Security | GuardDuty, WAF, KMS | €30–€80 |
| **Total** | | **€540–€1,430/mo** |

*See [Roadmap — Infrastructure Evolution](roadmap.md) for migration timeline and triggers.*

---

## Banking & Finance

### Corporate Banking

| Requirement | Recommendation | Notes |
|---|---|---|
| Business account | Erste Bank or Raiffeisen | Austrian Hausbank for credibility |
| Online banking | Required | Multi-currency support for EU expansion |
| Payment processing | Stripe | Already integrated; PCI DSS compliant |
| Accounting software | BMD or ProSaldo | Austrian tax-compliant (BAO) |

### Financial Controls

| Control | Implementation |
|---|---|
| Dual signature for amounts > €10,000 | Gesellschaftsvertrag provision |
| Monthly cash flow reporting | CEO + CFO review |
| Quarterly board updates | Investor reporting package |
| Annual audit | Voluntary until legally required (> €10M revenue or > 50 employees) |

---

## Insurance

### Recommended Policies

| Policy | Coverage | Priority |
|---|---|---|
| **Directors & Officers (D&O)** | Personal liability of Geschäftsführer and officers | High — at incorporation |
| **Professional liability (Berufshaftpflicht)** | Platform errors, incorrect valuations | High — at incorporation |
| **Cyber insurance** | Data breaches, ransomware, business interruption | High — at launch |
| **General liability (Betriebshaftpflicht)** | Third-party claims | Medium — at incorporation |
| **Legal expenses (Rechtsschutz)** | Legal disputes, regulatory proceedings | Medium — post-seed |

---

## Key Service Providers (Planned)

| Role | Type | Status |
|---|---|---|
| Corporate lawyer | Vienna startup law firm | 🔍 Shortlisting |
| Tax advisor (Steuerberater) | Vienna-based, startup experience | 🔍 Shortlisting |
| Notary (Notar) | Vienna, GmbH formations | 🔍 Shortlisting |
| Auditor (Wirtschaftsprüfer) | For voluntary audit post-Series A | 📋 Planned |
| Patent / trademark attorney | EUIPO filings | 📋 Planned |

---

## Timeline to Incorporation

```
Month 1                    Month 2                    Month 3
  │                          │                          │
  ├── Legal counsel engaged  ├── Notarization           ├── Trade license issued
  ├── Articles drafted       ├── Bank account opened    ├── Domain portfolio secured
  ├── IP audit completed     ├── Capital deposited      ├── Operations live under FlexCo
  │                          ├── Firmenbuch filing      ├── Operations live under FlexCo
  │                          ├── Tax + VAT registration │
  │                          ├── WiEReG filing          │
  └──────────────────────────┴──────────────────────────┘
```

**Target:** FlexCo fully operational within 90 days of initiating incorporation.

---

*Document status: V1 — For investor data room. Subject to legal review upon engagement of corporate counsel.*
