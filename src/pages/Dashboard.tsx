  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [carsResult, matchesResult, sellerOffersResult] = await Promise.all([
        ...
        supabase.from("offers").select("id, car_id, amount, status, current_round, created_at").eq("seller_id", session.user.id).in("status", ["pending", "countered"]).order("created_at", { ascending: false }),
      ]);
            ...
            <Card className="bg-secondary/50 border-border">
              ...
              {cars.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-muted-foreground text-sm mb-4">{t.dashboard.noCarsYet}</p>
          ...
          <div className="space-y-6">
            ...
            <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={6}>
              <Card className="bg-secondary/50 border-border">
                ...
                {recentShortlists.length === 0 ? (
                  <p className="p-6 text-sm text-muted-foreground text-center">{t.dashboard.noMatchesYet}</p>
                ) : (
                  <div className="divide-y divide-border">
