    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-display font-bold text-foreground">{t.onboarding.title}</h1>
            ...
            >
              {step === 1 && (
                <div className="space-y-6">
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.commuteDistance}</Label>
                  ...
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.familySize}</Label>
                    <div className="flex items-center gap-4">
                      <Button type="button" variant="outline" size="icon" className="border-border text-muted-foreground" onClick={() => setFamilySize(Math.max(1, familySize - 1))}>-</Button>
                      <span className="text-2xl font-display font-bold text-foreground w-10 text-center">{familySize}</span>
                      <Button type="button" variant="outline" size="icon" className="border-border text-muted-foreground" onClick={() => setFamilySize(Math.min(8, familySize + 1))}>+</Button>
                    </div>
                  </div>
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.usagePattern}</Label>
                  ...
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.parkingType}</Label>
                    <div className="grid grid-cols-2 gap-2">
              ...
              {step === 2 && (
                <div className="space-y-6">
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.budgetRange}</Label>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <span className="text-xs text-muted-foreground">{t.onboarding.budgetMin}</span>
                        <Input type="number" value={budgetMin} onChange={(e) => setBudgetMin(Number(e.target.value))} className="bg-background border-border text-foreground mt-1" />
                      </div>
                      <div>
                        <span className="text-xs text-muted-foreground">{t.onboarding.budgetMax}</span>
                        <Input type="number" value={budgetMax} onChange={(e) => setBudgetMax(Number(e.target.value))} className="bg-background border-border text-foreground mt-1" />
                      </div>
                    </div>
                  </div>
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.ownershipPref}</Label>
                    <div className="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.insuranceTolerance}</Label>
                    <div className="grid grid-cols-3 gap-2">
              ...
              {step === 3 && (
                <div className="space-y-6">
                  <div>
                    <Label className="text-muted-foreground text-sm mb-3 block">{t.onboarding.timingPref}</Label>
