...
const CarDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    supabase
      .from("cars")
      .select("id, make, model, year, mileage, fuel_type, transmission, body_type, color, power_hp, price, fair_value_price, condition_score, demand_score, equipment, description, photos, inspection_checklist, owner_id, first_registration_month, first_registration_year, pickerl_valid_month, pickerl_valid_year, warranty_type")
      .eq("id", id)
      .single()
      .then(async ({ data }) => {
        if (data) {
  ...
  const warrantyLabel = car.warranty_type === "manufacturer" 
    ? t.carDetail.warrantyManufacturer
    : car.warranty_type === "thirdParty"
      ? t.carDetail.warrantyThirdParty
      : t.carDetail.warrantyNone;
  ...
  const specs = [
    ...
    { icon: Calendar, label: t.carDetail.firstReg, value: firstRegLabel },
    { icon: CheckCircle2, label: t.carDetail.pickerl, value: pickerlLabel },
    { icon: Shield, label: t.carDetail.warranty, value: warrantyLabel },
  ];
