  ...
  return (
    <div className="max-h-[60vh] overflow-y-auto pr-1 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-xs mb-4">{t.carUpload.equipmentHint} — {equipment.length} selected</p>
      </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
            {items.map((item) => (
              ...
              >
                ...
                />
                <span className={equipment.includes(item) ? "text-primary" : "text-muted-foreground"}>{item}</span>
              </label>
            ))}
          </div>
        </div>
