...
const HowItWorksSection: React.FC = () => {
  ...
  return (
    <section id="how-it-works" className="py-12 sm:py-16 bg-muted">
      <div className="max-w-5xl mx-auto px-4">
        <motion.div className="text-center mb-12" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0}>
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-display font-black text-foreground">{t.howItWorks.title}</h2>
          {(t.howItWorks as any).subtitle && (
            <p className="text-muted-foreground text-base sm:text-lg mt-3">{(t.howItWorks as any).subtitle}</p>
          )}
        </motion.div>
        ...
        <motion.div className="text-center" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={4}>
          <Button
            size="lg"
            className="bg-orange text-orange-foreground hover:bg-orange/90 font-bold text-base px-8 py-6 rounded-lg shadow-md"
